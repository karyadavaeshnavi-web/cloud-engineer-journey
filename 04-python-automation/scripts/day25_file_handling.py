...
# Write to hello.txt
with open("reports/hello.txt", "w") as file:
    file.write("Hello Cloud Engineer")

# Append to hello.txt
with open("reports/hello.txt", "a") as file:
    file.write("\nPython Automation")

# Read hello.txt
with open("reports/hello.txt", "r") as file:
    print(file.read())

# Read server list
print("\n===== SERVERS =====")

with open("reports/servers.txt", "r") as file:
    for server in file:
        print(server.strip())
